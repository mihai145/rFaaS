...
namespace rdmalib {
    ...
    enum class QueueType { SEND, RECV };

    template <int Key = 8, int UserData = 8, int Secret = 16> struct PrivateData {

        PrivateData(uint32_t private_data = 0) : _private_data(private_data) {}
        ...
        uint32_t data() const { return _private_data; }
    ...
    struct Connection {
      ...
      public:
        ...
        int32_t post_write(ScatterGatherElement &&elems, const RemoteBuffer &buf, uint32_t immediate,
                           bool force_inline = false, bool solicited = false);
        int32_t post_read(ScatterGatherElement &&elems, const RemoteBuffer &rbuf, int32_t id = -1);
