...
namespace rdmalib {
    ...
    Connection::Connection(int rcv_buf_size, bool passive)
        : _id(nullptr), _qp(nullptr), _channel(nullptr), _req_count(0), _private_data(0), _passive(passive),
          _status(ConnectionStatus::UNKNOWN), _send_wcs(nullptr), _rcv_wcs(rcv_buf_size, nullptr) {
        inlining(false);
    ...
    Connection::Connection(Connection &&obj)
        : _id(obj._id), _qp(obj._qp), _channel(obj._channel), _req_count(obj._req_count),
          _private_data(obj._private_data), _passive(obj._passive), _status(obj._status),
          _send_wcs(std::move(obj._send_wcs)), _rcv_wcs(std::move(obj._rcv_wcs)), _send_flags(obj._send_flags) {
        obj._id = nullptr;
        obj._qp = nullptr;
        obj._req_count = 0;
    }

    RecvWorkCompletions &Connection::receive_wcs() { return _rcv_wcs; }

    SendWorkCompletions &Connection::send_wcs() { return _send_wcs; }

    int Connection::rcv_buf_size() const { return _rcv_wcs.rcv_buf_size(); }
    ...
    rdma_cm_id *Connection::id() const { return this->_id; }

    ibv_qp *Connection::qp() const { return this->_qp; }

    ibv_comp_channel *Connection::completion_channel() const { return this->_channel; }

    uint32_t Connection::private_data() const { return this->_private_data; }

    ConnectionStatus Connection::status() const { return this->_status; }

    void Connection::set_status(ConnectionStatus status) { this->_status = status; }

    void Connection::set_private_data(uint32_t private_data) { this->_private_data = private_data; }
    ...
    int32_t Connection::post_send(const ScatterGatherElement &elems, int32_t id, bool force_inline,
        ...
        if (ret) {
            spdlog::error("Post send unsuccesful, reason {} {}, sges_count {}, wr_id {}, wr.send_flags {}", errno,
                          strerror(errno), wr.num_sge, wr.wr_id, wr.send_flags);
            return -1;
        }
        SPDLOG_DEBUG("Post send succesfull, sges_count {}, sge[0].addr {}, sge[0].size {}, wr_id {}, wr.send_flags {}",
    ...
    int32_t Connection::_post_write(ScatterGatherElement &&elems, ibv_send_wr wr, bool force_inline,
                                    bool force_solicited, int32_t id) {
        ibv_send_wr *bad;
        wr.wr_id = id == -1 ? _req_count++ : id;
        wr.next = nullptr;
        wr.sg_list = elems.array();
        ...
        int ret = ibv_post_send(_qp, &wr, &bad);
        if (ret) {
            spdlog::error("Post write unsuccesful, reason {} {}, sges_count {}, wr_id {}, remote addr {}, remote rkey "
                          "{}, imm data {}",
                          ret, strerror(ret), wr.num_sge, wr.wr_id, wr.wr.rdma.remote_addr, wr.wr.rdma.rkey,
                          ntohl(wr.imm_data));
        ...
        if (wr.num_sge > 0)
            SPDLOG_DEBUG("Post write succesfull id: {}, sge size: {}, first lkey {} len {}, remote addr {}, remote "
                         "rkey {}, imm data {}",
                         wr.wr_id, wr.num_sge, wr.sg_list[0].lkey, wr.sg_list[0].length, wr.wr.rdma.remote_addr,
                         wr.wr.rdma.rkey, ntohl(wr.imm_data));
        else
            SPDLOG_DEBUG("Post write succesfull id: {}, remote addr {}, remote rkey {}, imm data {}", wr.wr_id,
                         wr.wr.rdma.remote_addr, wr.wr.rdma.rkey, ntohl(wr.imm_data));
        return static_cast<int32_t>(wr.wr_id);
    }
    ...
    int32_t Connection::post_write(ScatterGatherElement &&elems, const RemoteBuffer &rbuf, uint32_t immediate,
                                   bool force_inline, bool force_solicited) {
        ibv_send_wr wr;
        memset(&wr, 0, sizeof(wr));
    ...
    int32_t Connection::post_read(ScatterGatherElement &&elems, const RemoteBuffer &rbuf, int32_t id) {
        ibv_send_wr wr;
        memset(&wr, 0, sizeof(wr));
        wr.opcode = IBV_WR_RDMA_READ;
        wr.wr.rdma.remote_addr = rbuf.addr;
        wr.wr.rdma.rkey = rbuf.rkey;
        return _post_write(std::forward<ScatterGatherElement>(elems), wr, false, false, id);
    }

    int32_t Connection::post_cas(ScatterGatherElement &&elems, const RemoteBuffer &rbuf, uint64_t compare,
                                 uint64_t swap) {
        ibv_send_wr wr, *bad;
        memset(&wr, 0, sizeof(wr));
        wr.wr_id = _req_count++;
    ...
    int32_t Connection::post_atomic_fadd(ScatterGatherElement &&elems, const RemoteBuffer &rbuf, uint64_t add) {
        ...
        SPDLOG_DEBUG("Post atomic fadd succesfull id: {}, remote addr {}, remote rkey {}, val {}", wr.wr_id,
                     wr.wr.rdma.remote_addr, wr.wr.rdma.rkey, wr.wr.atomic.compare_add);
        return _req_count - 1;
    }
    ...
    std::tuple<ibv_wc *, int> Connection::poll_wc(QueueType type, bool blocking, int count) {
        ...
        do {
            ret =
                ibv_poll_cq(type == QueueType::RECV ? _qp->recv_cq : _qp->send_cq, count == -1 ? wc_size : count, wcs);
        } while (blocking && ret == 0);

        if (ret < 0) {
            spdlog::error("Failure of polling events from: {} queue! Return value {}, errno {}",
                          type == QueueType::RECV ? "recv" : "send", ret, errno);
            return std::make_tuple(nullptr, -1);
        }
        if (ret)
            for (int i = 0; i < ret; ++i) {
                if (wcs[i].status != IBV_WC_SUCCESS) {
                    spdlog::error("Queue {} Work Completion {}/{} finished with an error {}, {}",
                                  type == QueueType::RECV ? "recv" : "send", i + 1, ret, wcs[i].status,
                                  ibv_wc_status_str(wcs[i].status));
                }
                SPDLOG_DEBUG("Queue {} Ret {}/{} WC {} Status {}", type == QueueType::RECV ? "recv" : "send", i + 1,
                             ret, wcs[i].wr_id, ibv_wc_status_str(wcs[i].status));
            }
    ...
    void Connection::ack_events(ibv_cq *cq, int len) { ibv_ack_cq_events(cq, len); }
