...
namespace rdmalib {
    ...
    Address::Address(const std::string &ip, int port, bool passive) {
        ...
        impl::expect_zero(rdma_getaddrinfo(const_cast<char *>(ip.c_str()),
                                           const_cast<char *>(std::to_string(port).c_str()), &hints, &addrinfo));
        this->_port = port;
    }
    ...
    Address::Address(Address &&obj) : addrinfo(obj.addrinfo), hints(obj.hints), _port(obj._port) {
    ...
    void Address::set_port(uint32_t port) { this->_port = port; }

    uint32_t Address::port() const { return _port; }

    RDMAActive::RDMAActive() : _conn(nullptr), _ec(nullptr), _pd(nullptr), _is_connected(false) {}

    RDMAActive::RDMAActive(const std::string &ip, int port, int recv_buf, int max_inline_data, ibv_pd *pd)
        : _conn(nullptr), _addr(ip, port, false), _ec(nullptr), _pd(pd), _recv_buf(recv_buf), _is_connected(false) {
        // Size of Queue Pair
        // Maximum requests in send queue
        // FIXME: configurable -> parallel workers
        _cfg.attr.cap.max_send_wr = 128;
    ...
    void RDMAActive::allocate() {
        ...
        // spdlog::info("{} {} {} {}", ret, conn != nullptr, addr->ai_addr != nullptr, _addr.addrinfo->ai_dst_addr !=
        // nullptr);
        ////ret = rdma_resolve_addr(conn, addr->ai_addr, _addr.addrinfo->ai_dst_addr, 2000);
        ////spdlog::info("{} {} {} {} {}", ret, conn != nullptr, conn->verbs != nullptr, conn->pd != nullptr, conn->qp
        ///!= nullptr);

        // ret = rdma_bind_addr(conn, addr->ai_addr);
        // spdlog::info("{} {} {} {} {} {}", ret, errno, conn != nullptr, conn->verbs != nullptr, conn->pd != nullptr,
        // conn->qp != nullptr);
    ...
    bool RDMAActive::connect(uint32_t secret) {
        ...
        if (rdma_connect(_conn->id(), &_cfg.conn_param)) {
            ...
            spdlog::debug("[RDMAActive] Connection succesful to {}:{}, on device {}", _addr._port, _addr._port,
                          ibv_get_device_name(this->_conn->id()->verbs->device));
        }
    ...
    ibv_pd *RDMAActive::pd() const { return this->_pd; }

    Connection &RDMAActive::connection() { return *this->_conn; }

    bool RDMAActive::is_connected() const { return _is_connected; }

    RDMAPassive::RDMAPassive(const std::string &ip, int port, int recv_buf, bool initialize, int max_inline_data)
        : _addr(ip, port, true), _ec(nullptr), _listen_id(nullptr), _pd(nullptr), _recv_buf(recv_buf) {
        // Size of Queue Pair
        // FIXME: configurable -> parallel workers
        _cfg.attr.cap.max_send_wr = 128;
        _cfg.attr.cap.max_recv_wr = recv_buf;
    ...
    RDMAPassive::RDMAPassive(RDMAPassive &&obj)
        : _cfg(std::move(obj._cfg)), _addr(std::move(obj._addr)), _ec(std::move(obj._ec)),
          _listen_id(std::move(obj._listen_id)), _pd(std::move(obj._pd)), _recv_buf(obj._recv_buf),
    ...
    void RDMAPassive::allocate() {
        ...
        spdlog::info("Listening on device {}, port {}", ibv_get_device_name(this->_listen_id->verbs->device),
                     this->_addr._port);
        SPDLOG_DEBUG("[RDMAPassive]: listening id {}, protection domain {}", fmt::ptr(this->_listen_id), _pd->handle);
    }

    ibv_pd *RDMAPassive::pd() const { return this->_pd; }

    uint32_t RDMAPassive::listen_port() const { return this->_addr.port(); }
    ...
    std::tuple<Connection *, ConnectionStatus> RDMAPassive::poll_events(/*bool share_cqs*/) {
        ...
        SPDLOG_DEBUG("[RDMAPassive] received event: {}, status: {}", rdma_event_str(event->event), event->status);
        ...
        case RDMA_CM_EVENT_CONNECT_REQUEST: {
            ...
            SPDLOG_DEBUG("[RDMAPassive] Creating QP: id {} pd {} listener pd {}", fmt::ptr(connection->id()),
                         fmt::ptr(_pd), fmt::ptr(this->_listen_id->pd));
            ...
            SPDLOG_DEBUG("[RDMAPassive] Using CQ for creating a QP: send {} recv {}", fmt::ptr(_cfg.attr.send_cq),
                         fmt::ptr(_cfg.attr.recv_cq));
            ...
            SPDLOG_DEBUG("[RDMAPassive] Created connection id {} qpnum {} qp {} send {} recv {}, completion channel {}",
        ...
        case RDMA_CM_EVENT_ESTABLISHED:
            SPDLOG_DEBUG("[RDMAPassive] Connection is established for id {}, and connection {}", fmt::ptr(event->id),
                         fmt::ptr(event->id->context));
            connection = reinterpret_cast<Connection *>(event->id->context);
            status = ConnectionStatus::ESTABLISHED;
            break;
        case RDMA_CM_EVENT_DISCONNECTED:
            SPDLOG_DEBUG("[RDMAPassive] Disconnect for id {}, and connection {}", fmt::ptr(event->id),
                         fmt::ptr(event->id->context));
            connection = reinterpret_cast<Connection *>(event->id->context);
            // connection->close();
        ...
        case RDMA_CM_EVENT_ROUTE_RESOLVED:
            SPDLOG_DEBUG("[RDMAPassive] Unexpected event: {}, status: {}\n", rdma_event_str(event->event),
                         event->status);
            break;
