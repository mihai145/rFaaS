...
namespace server {
    ...
    FastExecutors::FastExecutors(std::string client_addr, int port, int func_size, int numcores, int msg_size,
                                 int recv_buf_size, int max_inline_data, bool use_multiprocessing, int pin_threads,
                                 const executor::ManagerConnection &mgr_conn)
        : _closing(false), _function_size(func_size), _msg_size(msg_size), _recv_buf_size(recv_buf_size),
          _max_inline_data(max_inline_data), _numcores(numcores), _max_repetitions(0),
          _use_multiprocessing(use_multiprocessing), _pin_threads(pin_threads)
    //_mgr_conn(mgr_conn)
    {
        // Reserve place to ensure that no reallocations happen
        _workers_data.reserve(numcores);
        for (int i = 0; i < numcores; ++i)
            _workers_data.emplace_back(client_addr, port, i, func_size, msg_size, recv_buf_size, max_inline_data,
                                       mgr_conn);
    }
    ...
    void FastExecutors::close() {
        ...
        if (!_use_multiprocessing) {
            for (auto &thread : _workers_data)
                spdlog::info("Thread {} Repetitions {} Avg time {} ms", thread.id, thread.repetitions,
                             static_cast<double>(thread._accounting.total_execution_time) / thread.repetitions /
                                 1000.0);
        } else {
    ...
    void FastExecutors::allocate_workers(int timeout, int iterations) {
        if (_use_multiprocessing) {
            for (int i = 0; i < _numcores; ++i) {
                ...
                if (pid == 0) {
                    const std::string port = std::to_string(_workers_data[i].port);
                    const std::string id = std::to_string(_workers_data[i].id);
                    const std::string functions_size = std::to_string(_function_size);
                    const std::string buf_size = std::to_string(_msg_size);
                    const std::string recv_buffer_size = std::to_string(_recv_buf_size);
                    const std::string max_inline_data = std::to_string(_max_inline_data);
                    const std::string mgr_conn_port = std::to_string(_workers_data[i]._mgr_conn.port);
                    const std::string mgr_conn_secret = std::to_string(_workers_data[i]._mgr_conn.secret);
                    const std::string mgr_conn_r_addr = std::to_string(_workers_data[i]._mgr_conn.r_addr);
                    const std::string mgr_conn_r_key = std::to_string(_workers_data[i]._mgr_conn.r_key);
                    const std::string timeout_str = std::to_string(timeout);
                    const std::string iterations_str = std::to_string(iterations);
                    const char *argv[] = {"worker",
                                          "--addr",
                                          _workers_data[i].addr.c_str(),
                                          "--port",
                                          port.c_str(),
                                          "--id",
                                          id.c_str(),
                                          "--functions_size",
                                          functions_size.c_str(),
                                          "--buf_size",
                                          buf_size.c_str(),
                                          "--recv_buffer_size",
                                          recv_buffer_size.c_str(),
                                          "--max_inline_data",
                                          max_inline_data.c_str(),
                                          "--mgr_conn_addr",
                                          _workers_data[i]._mgr_conn.addr.c_str(),
                                          "--mgr_conn_port",
                                          mgr_conn_port.c_str(),
                                          "--mgr_conn_secret",
                                          mgr_conn_secret.c_str(),
                                          "--mgr_conn_r_addr",
                                          mgr_conn_r_addr.c_str(),
                                          "--mgr_conn_r_key",
                                          mgr_conn_r_key.c_str(),
                                          "--timeout",
                                          timeout_str.c_str(),
                                          "--iterations",
                                          iterations_str.c_str(),
                                          nullptr};
        ...
        for (int i = 0; i < _numcores; ++i) {
            ...
            _threads.emplace_back(&Worker::thread_work, &_workers_data[i], timeout);
            ...
            if (pin_threads != -1) {
                ...
                rdmalib::impl::expect_zero(
                    pthread_setaffinity_np(_threads[i].native_handle(), sizeof(cpu_set_t), &cpuset));
            }
        }
    }
    ...
    //    int usec = (end.tv_sec - _start_timestamps[id].tv_sec) * 1000000 + (end.tv_usec -
    //    _start_timestamps[id].tv_usec); sum += usec; SPDLOG_DEBUG("Thread {} loops again!", id);
    //  }
    //  SPDLOG_DEBUG("Thread {} exits!", id);
